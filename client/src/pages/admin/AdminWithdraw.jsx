...
export default function AdminWithdraw() {
  ...
  const load = () => {
    setLoading(true);
    adminService.getTransactions({ type: 'withdraw', status: 'requested', page, limit: PAGE_SIZE })
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <span className="text-sm font-bold text-slate-700">{txns.length} Pending Withdrawal Requests</span>
      </div>
      ...
      {loading ? <Loader /> : (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-slate-500 font-bold text-xs uppercase tracking-wide">
                <tr>
                  ...
                  <th className="text-left px-4 py-3">Account / UPI</th>
              ...
              <tbody className="divide-y divide-slate-50">
                {txns.map((t) => (
                  <tr key={t._id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-right font-bold text-red-500">₹{t.amount}</td>
                    <td className="px-4 py-3 text-slate-400 text-xs max-w-[180px] truncate">{t.description || t.note || '—'}</td>
            ...
            {!txns.length && <div className="text-center py-12 text-slate-400 text-sm">No pending withdrawal requests</div>}
          </div>
