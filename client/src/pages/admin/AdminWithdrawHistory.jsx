...
export default function AdminWithdrawHistory() {
  ...
  const load = () => {
    setLoading(true);
    adminService.getTransactions({ type: 'withdraw', status, page, limit: PAGE_SIZE })
      .then((data) => {
      ...
      {loading ? <Loader /> : (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            ...
            {!txns.length && <div className="text-center py-12 text-slate-400 text-sm">No withdrawal history found</div>}
          </div>
